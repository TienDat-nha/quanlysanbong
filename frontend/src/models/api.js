 ...
 * api.js
 * ======
 * Model quản lý các API calls tới backend
 *
 * Chức năng:
 * - Xác định base URL của API
 * - Cung cấp các hàm gọi API cho authentication (login, register, OTP verification)
 * - Cung cấp các hàm gọi API cho quản lý người dùng
 * - Cung cấp các hàm gọi API cho quản lý sân (fields)
 * - Cung cấp các hàm gọi API cho quản lý đặt sân (bookings)
 * - Cung cấp các hàm gọi API cho quản lý thanh toán (payments)
 * - Xử lý errors từ API
 * - Tự động deteect API base URL (local development hoặc production)
 *
 * Hàm chính:
 * - loginUser(): Đăng nhập người dùng
 * - registerUser(): Đăng ký người dùng
 * - requestRegisterOtp(), verifyRegisterOtp(): Quản lý OTP đăng ký
 * - getMe(): Lấy thông tin người dùng hiện tại
 * - getPublicUsers(), createPublicUser(), updatePublicUser(), deletePublicUser(): Quản lý người dùng
 * - getAdminFields(), approveAdminField(), rejectAdminField(), lockAdminField(), unlockAdminField(): Quản lý sân
 * - getPublicBookings(), getAdminBookings(), getOwnerBookings(): Quản lý đặt sân
 * - getPublicPayments(): Lấy danh sách thanh toán
 */
...
const API_BASE_URL = ensureApiBaseUrl(
  process.env.REACT_APP_API_URL || getDefaultApiOrigin(),
);
const API_BASE_URL_CANDIDATES = (() => {
  const candidates = new Set();
  const configuredApiBaseUrl = ensureApiBaseUrl(
  ...
  if (typeof window !== "undefined") {
    ...
    if (isLocalHostname(hostname)) {
      candidates.add(
        ensureApiBaseUrl(`http://${hostname || "localhost"}:5555`),
      );
      candidates.add(
        ensureApiBaseUrl(`http://${hostname || "localhost"}:5000`),
      );
    }
  }
...
const isDataImageLikeUrl = (value) =>
...
const API_ASSET_ORIGIN =
  [API_BASE_URL, ...API_BASE_URL_CANDIDATES]
...
const resolveApiAssetUrl = (value) => {
  ...
  if (
    !normalizedValue ||
    isDataImageLikeUrl(normalizedValue) ||
...
const BOOKING_DASHBOARD_PATHS = (params = {}) => {
  ...
  const recentLimit = Number.parseInt(
    String(params?.recentLimit || "").trim(),
    10,
  );
  const managedLimit = Number.parseInt(
    String(params?.managedLimit || "").trim(),
    10,
  );
  ...
  try {
    if (nextFieldIds.length === 0) {
      ...
      window.localStorage.setItem(
        KNOWN_FIELD_IDS_STORAGE_KEY,
        JSON.stringify(nextFieldIds),
      );
    }
  } catch (_error) {
    return nextFieldIds;
...
const buildPersistableFieldSnapshots = (
  snapshots = {},
  ...
  Object.entries(snapshots || {}).forEach(([key, value]) => {
    ...
    nextSnapshots[snapshotKey] = {
      ...value,
      coverImage:
        stripDataImages && coverImage.startsWith("data:") ? "" : coverImage,
      images: stripDataImages
...
const forgetKnownFieldId = (fieldId) =>
  setStoredKnownFieldIds(
    getStoredKnownFieldIds().filter(
      (item) => item !== String(fieldId || "").trim(),
    ),
  );
...
const getRouteErrorFromHtml = (value) => {
  ...
  const match = normalizedValue.match(
    /Cannot\s+(GET|POST|PUT|DELETE|PATCH)\s+([^<\s]+)/i,
  );
...
const getNestedValue = (source, path) =>
  String(path || "")
    ...
    .reduce(
      (currentValue, key) =>
        currentValue == null ? undefined : currentValue[key],
      source,
    );
...
const getMessageFromBody = (bodyData, rawBodyText) => {
  ...
  const message = pickFirstValue({ bodyData, normalizedBody }, [
    "bodyData.message",
    "bodyData.error.message",
    "normalizedBody.message",
    "normalizedBody.error.message",
  ]);
  const detail = pickFirstValue({ bodyData, normalizedBody }, [
    "bodyData.data",
    "bodyData.error.data",
    "normalizedBody.data",
  ...
  if (typeof detail === "string" && detail.trim()) {
    if (
      ...
      isGenericInvalidApiMessage(message)
    ) {
      return detail.trim();
    }
  }
...
const normalizeSubFieldItem = (subField, index = 0) => {
  ...
  const name = String(
    subField.name || subField.subFieldName || `San ${index + 1}`,
  ).trim();
  ...
  return {
    ...
    key:
      normalizeSubFieldKeyValue(key, `sub-field-${index + 1}`) ||
      `sub-field-${index + 1}`,
    name,
    type: normalizeFieldType(
      subField.type || subField.fieldType || subField.subFieldType || "",
    ),
    pricePerHour: Number(
    ...
    openHours: normalizeOpenHoursValue(
      subField.openHours || subField.timeRange || subField.openingHours || "",
      "",
    ),
  };
};
...
const normalizeFieldItem = (field) => {
  ...
  return {
    ...
    type: normalizeFieldType(
      field.type || field.fieldType || field.category || "",
    ),
    openHours: normalizeOpenHoursValue(
      field.openHours ||
        field.timeRange ||
    ...
    images:
      Array.isArray(field.images) && field.images.length > 0
        ...
        : Array.isArray(field.galleryImages) && field.galleryImages.length > 0
          ? field.galleryImages
              .map((item) => resolveApiAssetUrl(item))
              .filter(Boolean)
...
const mergeFieldSources = (field, snapshot) => {
  ...
  return normalizeFieldItem({
    ...
    district: pickFirstNonEmptyString(
      field?.district,
      field?.area,
      snapshot.district,
    ),
    ...
    article: pickFirstNonEmptyString(
      field?.article,
      field?.description,
      snapshot.article,
    ),
...
const rememberFieldSnapshot = (field) => {
  ...
  const nextSnapshots = {
    ...
    [normalizedField.id]: {
      ...
      images: Array.isArray(normalizedField.images)
        ? normalizedField.images
        : [],
      subFields: Array.isArray(normalizedField.subFields)
        ? normalizedField.subFields
        : [],
...
const normalizeTimeSlotItem = (slot, index = 0) => {
  ...
  const startTime = normalizeTimeString(
    slot.startTime || slot.start || slot.from || slot.timeStart,
  );
  const endTime = normalizeTimeString(
    slot.endTime || slot.end || slot.to || slot.timeEnd,
  );
...
const deriveBookingExpiredAt = (
  booking,
  holdMinutes = DEFAULT_BOOKING_HOLD_MINUTES,
) => {
  ...
  return new Date(
    createdAt.getTime() + Math.max(Number(holdMinutes) || 0, 0) * 60 * 1000,
  ).toISOString();
};
...
const normalizeBookingItem = (booking) => {
  ...
  const subField = normalizeSubFieldItem(
    booking.subField || booking.subFieldId,
  );
  const timeSlot = normalizeTimeSlotItem(
    booking.timeSlotInfo || booking.timeSlot || booking.timeSlotId,
  );
  ...
  const totalPrice = Number(
    booking.totalPrice || booking.price || booking.amount || 0,
  );
  const depositAmount = Number(
    booking.depositAmount || booking.paidAmount || booking.payment?.amount || 0,
  );
  const rawRemainingAmount =
    booking.remainingAmount ?? booking.payment?.remainingAmount ?? null;
  ...
  return {
    ...
    fieldDistrict: String(
      booking.fieldDistrict || field?.district || "",
    ).trim(),
    ...
    timeSlot: String(
      booking.timeSlotLabel || booking.timeSlot || timeSlot?.timeSlot || "",
    ).trim(),
    ...
    depositMethod: String(
      booking.depositMethod || booking.payment?.method || "",
    ).trim(),
    paymentType,
    depositPaid: Boolean(booking.depositPaid || isPaidPaymentStatus),
    fullyPaid: Boolean(
      booking.fullyPaidAt ||
      String(booking.status || booking.bookingStatus || "")
        .trim()
...
const normalizePaymentItem = (payment) => {
  ...
  return {
    ...
    paymentType: String(payment.paymentType || payment.type || "")
      .trim()
      .toUpperCase(),
    status: String(payment.status || payment.paymentStatus || "")
      .trim()
      .toUpperCase(),
    ...
    for (const key of preferredKeys) {
      if (key in value) {
        ...
        if (
          ...
          (nestedArray.length > 0 || Array.isArray(value[key]))
        ) {
          return nestedArray;
        }
      }
  ...
  for (const apiBaseUrl of apiBaseCandidates) {
    ...
    try {
      ...
      const networkError = new Error(
        buildApiUnavailableMessage(`${method} ${apiBaseUrl}${path}`),
      );
    ...
    if (!response.ok) {
      ...
      apiError.isRouteMissing = isRouteMissingResponse(
        response.status,
        rawBodyText,
      );
...
const isRenderHostedApi = () =>
  /onrender\.com/i.test(String(API_BASE_URL || ""));

const shouldRetryTransientApiError = (error) =>
  isRenderHostedApi() &&
  RENDER_TRANSIENT_ERROR_STATUSES.has(Number(error?.status || 0));
...
const buildBookingPaymentShape = (booking, payment) => {
  ...
  const mergedBooking = normalizedBooking
    ? {
        ...
        paymentStatus:
          normalizedBooking.paymentStatus || normalizedPayment?.status || "",
        depositStatus:
          normalizedBooking.depositStatus || normalizedPayment?.status || "",
        depositMethod:
          normalizedBooking.depositMethod || normalizedPayment?.method || "",
        depositAmount:
          normalizedBooking.depositAmount ||
  ...
  return {
    ...
    paymentMethods: {
      staticTransfer: {
        ...
        message: normalizedPayment
          ? "Don dat san nay da co yeu cau thanh toan."
          : "",
      },
      vnpay: {
        enabled: Boolean(normalizedPayment?.id),
        message: normalizedPayment?.id
          ? ""
          : "Can tao yeu cau thanh toan truoc khi lay QR.",
      },
      momo: {
    ...
    staticTransfer: {
      ...
      accountNumber:
        normalizedPayment?.id || normalizedBooking?.paymentId || "",
...
const normalizeOtpRequestError = (
  ...
  const otpRetryMatch = message.match(
    /please\s+wait\s+(\d+)s\s+before\s+requesting\s+a\s+new\s+otp/i,
  );

  if (status >= 500 || normalizedMessage === "da co loi xay ra") {
...
export const requestRegisterOtp = async (payload = {}) => {
  ...
  let response;
  try {
    response = await requestFirstSuccess(SEND_OTP_PATHS, {
      method: "POST",
      body: JSON.stringify({
        email,
    ...
    throw new Error(
      normalizeOtpRequestError(
        error,
        "Khong the gui OTP luc nay. Vui long thu lai sau.",
      ),
    );
  }
  const data =
    getObjectFromResponse(response, ["data"]) ||
  ...
  const expiresInMinutes = Number(
    data?.expiresInMinutes || data?.expiresMinutes || 5,
  );
...
export const verifyRegisterOtp = async (payload = {}) => {
  ...
  let response;
  try {
    response = await requestFirstSuccess(VERIFY_OTP_PATHS, {
      method: "POST",
      body: JSON.stringify({
        email,
        otp,
    ...
    throw new Error(
      normalizeOtpRequestError(
        error,
        "Khong the xac nhan OTP luc nay. Vui long thu lai sau.",
      ),
    );
  }
...
export const getManagedUserOtpSetupMessage = () =>
  getManagedUserOtpConfigMessage();
...
export const loginUser = async (payload) => {
  const response = await requestFirstSuccess(LOGIN_PATHS, {
    method: "POST",
    body: JSON.stringify({
      username: String(payload?.email || payload?.username || "")
        .trim()
        .toLowerCase(),
      password: String(payload?.password || ""),
    }),
...
export const getPublicUsers = async () => {
  ...
  return {
    users: getArrayFromResponse(response, ["users"])
      .map((item) => normalizeAuthUser(item))
      .filter(Boolean),
  };
};
...
const buildAdminUserPayload = (
  payload = {},
  { defaultRole = "", includePassword = false } = {},
) => {
  const nextPayload = {
  ...
  const role = String(payload?.role || defaultRole || "")
    .trim()
    .toUpperCase();
  if (role) {
...
export const getOwnerRequests = async (token) => {
  ...
  return {
    requests: getArrayFromResponse(response, ["requests", "users"])
      .map((item) => normalizeAuthUser(item))
      .filter(Boolean),
    message: String(response?.message || "").trim(),
  };
...
export const deleteUserByAdmin = async (token, userId) =>
  request(
    `/user/deleteUserByAdmin/${encodeURIComponent(String(userId || "").trim())}`,
    {
      method: "DELETE",
      headers: createTokenHeaders(token),
      body: JSON.stringify({}),
...
export const getTimeSlots = async () => {
  ...
  return {
    timeSlots: getArrayFromResponse(response, ["timeSlots"])
      .map((item, index) => normalizeTimeSlotItem(item, index))
      .filter(Boolean),
  };
};
...
export const getTimeSlot = async (timeSlotId, token = "") => {
  ...
  return {
    timeSlot:
      normalizeTimeSlotItem(
        getObjectFromResponse(response, ["timeSlot", "item"]),
...
export const getSubFieldsByField = async (fieldId, token = "") => {
  ...
  const response = await requestWithTransientRetry(
    `/subField/getByField/${encodedFieldId}`,
    token
      ? {
          headers: createTokenHeaders(token),
        }
...
const getFieldByConfiguredIds = async (fieldIds = [], token = "") => {
  const nextFieldIds = buildUniqueStringList(fieldIds);
  const results = await Promise.allSettled(
    nextFieldIds.map((fieldId) => getFieldById(fieldId, token)),
  );
...
export const getFields = async (token = "") => {
  const knownFieldIds = buildUniqueStringList([
    ...CONFIGURED_FIELD_IDS,
    ...getStoredKnownFieldIds(),
  ]);
  ...
  if (lastError) {
    if (knownFieldIds.length > 0) {
      const rememberedFields = await getFieldByConfiguredIds(
        knownFieldIds,
        token,
      );
    ...
    if (storedSnapshotFields.length > 0) {
      const hydratedSnapshotFields = await attachSubFieldsToFields(
        storedSnapshotFields,
        token,
      );
...
export const getAdminDashboard = async (token, params = {}) => {
  ...
  const payload =
    getObjectFromResponse(response, ["dashboard"]) ||
    unwrapResponseData(response) ||
    {};
  ...
  return {
    ...
    availabilityDate: String(
      payload?.availabilityDate || params?.date || "",
    ).trim(),
    message: String(response?.message || "").trim(),
  };
};
...
const PENDING_PAYMENT_CONFIRM_STATUSES = new Set([
  "",
  "PENDING",
  "WAITING",
  "PROCESSING",
...
const isPendingPaymentForConfirmation = (payment) =>
  PENDING_PAYMENT_CONFIRM_STATUSES.has(
...
const getLatestPaymentByBooking = async (
  token,
  bookingId,
  paymentType = "",
) => {
  const response = await request(
    `/payment/getPaymentByBooking/${encodeURIComponent(bookingId)}`,
    {
      headers: createTokenHeaders(token),
    },
  );
  ...
  const typedPayments = normalizedPaymentType
    ? payments.filter((item) => {
        const itemPaymentType = normalizeAdminPaymentType(
          item?.paymentType,
          "",
        );
  ...
  const pendingPayment = typedPayments.find((item) =>
    isPendingPaymentForConfirmation(item),
  );
  return pendingPayment || typedPayments[0] || payments[0] || null;
};

const confirmBookingPaymentByBookingId = async (
  token,
  bookingId,
  options = {},
) => {
  const normalizedBookingIds = normalizeAdminBookingIds(
    bookingId,
    options?.bookingIds,
  );
  const normalizedBookingId =
    normalizedBookingIds[0] || String(bookingId || "").trim();
  ...
  let payment = isGroupedPayment
    ? null
    : await getLatestPaymentByBooking(
        token,
        normalizedBookingId,
...
export const confirmAdminBookingDeposit = async (
  token,
  bookingId,
  bookingIds = [],
) =>
...
export const confirmAdminBookingPayment = async (
  token,
  bookingId,
  amount = null,
  bookingIds = [],
...
export const createAdminSubField = async (token, payload) => {
  ...
  return {
    subField:
      normalizeSubFieldItem(
        getObjectFromResponse(response, ["subField", "item"]),
...
export const updateAdminSubField = async (token, subFieldId, payload) => {
  const response = await request(
    `/subField/updateSubField/${encodeURIComponent(String(subFieldId || "").trim())}`,
    {
      method: "POST",
      headers: createTokenHeaders(token),
      body: JSON.stringify(payload),
...
export const deleteAdminSubField = async (token, subFieldId) =>
  request(
    `/subField/deleteSubField/${encodeURIComponent(String(subFieldId || "").trim())}`,
    {
      method: "POST",
      headers: createTokenHeaders(token),
    },
...
const syncAdminFieldSubFields = async (
  token,
  fieldId,
  payload = {},
  currentSubFields = [],
) => {
  const desiredSubFields = (
    Array.isArray(payload?.subFields) ? payload.subFields : []
  )
    .map((subField, index) => ({
  ...
  const existingSubFields = (
    Array.isArray(currentSubFields) ? currentSubFields : []
  )
...
export const updateAdminField = async (token, fieldId, payload) => {
  const response = await request(
    `/field/updateField/${encodeURIComponent(fieldId)}`,
    {
      method: "POST",
      headers: createTokenHeaders(token),
      body: JSON.stringify(buildFieldMutationPayload(payload)),
  ...
  try {
    ...
    currentSubFields = Array.isArray(subFieldData?.subFields)
  ...
  if (hydratedField?.id || fieldId) {
    ...
    rememberFieldSnapshot(
      hydratedField || { ...payload, id: fieldId, _id: fieldId, fieldId },
    );
  }
...
export const lockAdminField = async (token, fieldId) =>
  requestWithTransientRetry(
    `/field/lockField/${encodeURIComponent(String(fieldId || "").trim())}`,
    {
      method: "POST",
      headers: createTokenHeaders(token),
      body: JSON.stringify({}),
...
export const unlockAdminField = async (token, fieldId) =>
  requestWithTransientRetry(
    `/field/unlockField/${encodeURIComponent(String(fieldId || "").trim())}`,
    {
      method: "POST",
      headers: createTokenHeaders(token),
      body: JSON.stringify({}),
...
export const createBooking = async (token, payload) => {
  ...
  const booking = attachBookingHoldInfo(
    getObjectFromResponse(response, ["booking", "item"]) ||
      unwrapResponseData(response),
    response,
  );
...
export const getBookedSlots = async (subFieldId, date, token = "") => {
  ...
  const bookingSlotItems = getArrayFromResponse(response, [
    "bookings",
    "bookedSlots",
    "slots",
    "items",
  ]);
  const bookedTimeSlotItems = getArrayFromResponse(response, [
    "bookedTimeSlotIds",
    "timeSlotIds",
  ]);
  const slotRecords = (
    bookingSlotItems.length > 0 ? bookingSlotItems : bookedTimeSlotItems
  )
    .map((item, index) => {
      ...
      return {
        ...
        _id: slotRecordId,
        subFieldId: String(
          normalizedBooking?.subFieldId || normalizedSubFieldId,
        ).trim(),
        ...
        timeSlot,
        timeSlotInfo:
          normalizedBooking?.timeSlotInfo || normalizedTimeSlot || null,
        status: String(
          normalizedBooking?.status || item?.status || "PENDING",
        ).trim(),
  ...
  return {
    timeSlotIds: slotRecords
      .map((item) => String(item?.timeSlotId || "").trim())
      .filter(Boolean),
    slotRecords,
    message: String(response?.message || "").trim(),
  };
...
export const getMyBookings = async (token) => {
  ...
  return {
    bookings: getArrayFromResponse(response, [
      "bookings",
      "items",
      "rows",
      "records",
  ...
  const response = await request("/payment/createPayment", {
    ...
    body: JSON.stringify({
      ...
      method: String(method || "CASH")
        .trim()
        .toUpperCase(),
      paymentType: String(paymentType || "DEPOSIT")
        .trim()
        .toUpperCase(),
...
export const getMyPayments = async (token) => {
  ...
  return {
    payments: getArrayFromResponse(response, ["payments", "items"])
      .map((item) => normalizePaymentItem(item))
      .filter(Boolean),
  };
};
...
export const checkPaymentStatus = async (token, paymentId) => {
  const response = await request(
    `/payment/checkStatus/${encodeURIComponent(String(paymentId || "").trim())}`,
    {
      headers: createTokenHeaders(token),
    },
  );
...
export const getPaymentQr = async (token, paymentId) => {
  const response = await request(
    `/payment/getQR/${encodeURIComponent(paymentId)}`,
    {
      headers: createTokenHeaders(token),
    },
  );
