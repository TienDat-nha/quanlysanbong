...
import { createBookingRoute } from "../../models/routeModel"
...
const getFieldModerationState = (field) => {
  const status = String(field?.approvalStatus || field?.status || field?.fieldStatus || "")
    .trim()
    .toUpperCase()
...
const fieldLink = (manualBookingPath, field) => {
  const slug = String(field?.slug || "").trim()
  if (slug) {
    return createBookingRoute(slug, field?.id)
  }

  return `${manualBookingPath}?fieldId=${encodeURIComponent(String(field?.id || "").trim())}`
}
    ...
    <form className="formCard ownerCreateFieldForm" onSubmit={handleSubmit}>
      ...
      <section className="ownerFormSection">
        ...
        <div className="ownerFormGrid">
          <InputField id="admin-field-name" label="Tên sân" error={formErrors?.name}>
            <input
              id="admin-field-name"
          ...
          <InputField id="admin-field-address" label="Địa chỉ" error={formErrors?.address} wide>
            <input
              id="admin-field-address"
              className={controlClass(formErrors?.address)}
              value={form.address}
              onChange={(event) => handleFieldChange("address", event.target.value)}
            />
          </InputField>
          <InputField id="admin-field-district" label="Khu vực" error={formErrors?.district}>
            <input
              id="admin-field-district"
              className={controlClass(formErrors?.district)}
              value={form.district}
              onChange={(event) => handleFieldChange("district", event.target.value)}
            />
          </InputField>
          <InputField id="admin-field-type" label="Loại sân mặc định" error={formErrors?.type}>
            <select
              id="admin-field-type"
              className={controlClass(formErrors?.type)}
              value={form.type}
          ...
          <InputField id="admin-field-open-hours" label="Giờ mở cửa" error={formErrors?.openHours}>
            <input
              id="admin-field-open-hours"
              className={controlClass(formErrors?.openHours)}
          ...
          <InputField
            ...
            label="Giá mặc định theo giờ"
            error={formErrors?.pricePerHour}
          >
            <input
              id="admin-field-price"
              className={controlClass(formErrors?.pricePerHour)}
              type="number"
              min="1000"
              step="1000"
      ...
      <section className="ownerFormSection">
        ...
        <div className="ownerUploadGrid">
          <div className="ownerUploadBox">
            <InputField id="admin-field-cover" label="Ảnh đại diện" error="">
              <input
                id="admin-field-cover"
                type="file"
                accept="image/png,image/jpeg,image/webp,image/gif"
                onChange={async (event) => {
                  await handleCoverImageUpload(event.target.files?.[0])
                  event.target.value = ""
                }}
              />
            </InputField>
          ...
          <div className="ownerUploadBox">
            <InputField id="admin-field-images" label="Thư viện ảnh" error="">
              <input
            ...
            {form.galleryImages.length > 0 ? (
              <div className="adminUploadPreviewGrid ownerGalleryGrid">
                {form.galleryImages.map((imageUrl, index) => (
                  <article
                    className="adminUploadPreviewCard ownerUploadPreviewCard"
                    key={`${imageUrl}-${index}`}
                  >
                    <img
                      src={imageUrl}
        ...
        <InputField id="admin-field-article" label="Mô tả sân" error="" wide>
          <textarea
            id="admin-field-article"
            className="ownerFormControl"
            rows={4}
            value={form.article}
              ...
              <article className="ownerSubFieldCard" key={subField.id}>
                ...
                <div className="ownerFormGrid ownerFormGrid--subfield">
                  ...
                  >
                    <input
                      id={`admin-subfield-name-${subField.id}`}
                      className={controlClass(subErrors.name)}
                      value={subField.name}
                      onChange={(event) =>
                  ...
                  <InputField
                    ...
                    label="Loại sân"
                    error={subErrors.type}
                  >
                    <select
                      id={`admin-subfield-type-${subField.id}`}
                      className={controlClass(subErrors.type)}
                      value={subField.type}
                      onChange={(event) =>
                  ...
                  <InputField
                    ...
                    label="Giá theo giờ"
                    error={subErrors.pricePerHour}
                  >
                    <input
                      id={`admin-subfield-price-${subField.id}`}
                      className={controlClass(subErrors.pricePerHour)}
                      type="number"
                      min="1000"
                      step="1000"
      ...
      <div className="ownerFormActions">
        ...
        {isEditingField && (
          <button className="outlineBtnInline" type="button" onClick={handleCancelFieldEdit}>
            Hủy chỉnh sửa
          </button>
        )}
      </div>
...
const FieldListSection = ({
  ...
  const pendingCount = fields.filter((field) => getFieldModerationState(field) === "PENDING").length
  ...
  return (
    <section className="ownerSectionCard ownerSectionCard--list" id={fieldListSectionId}>
      <div className="ownerSectionHeading">
        <div className="ownerSectionTitle">
          <p className="ownerSectionEyebrow">
            {isAdminPortal ? "Yêu cầu từ Chủ sân" : "Kho sân đang quản lý"}
          </p>
          <h2>{isAdminPortal ? "Xác nhận tạo và sửa sân của Chủ sân" : "Danh sách sân của bạn"}</h2>
          {isAdminPortal && (
            <p className="helperText">
              {pendingCount > 0
                ? `Hiện có ${pendingCount} sân đang chờ admin duyệt hoặc xử lý.`
                : "Danh sách này hiển thị các sân Chủ sân đã gửi để admin duyệt, khóa hoặc chỉnh sửa."}
            </p>
          )}
        </div>
        {isOwnerPortal && (
      ...
      {loading ? (
        ...
        <p className="usersEmptyState">
          {isAdminPortal
            ? "Chưa có yêu cầu tạo hoặc sửa sân nào từ Chủ sân."
          ...
          {fields.map((field) => {
            ...
            const manualHint =
              !canManualBook && isOwnerPortal
                ...
                : ""
            const publicBookingUrl = createPublicBookingUrl(publicOrigin, field.slug, field.id)
              ...
              <article className="ownerFieldCard" key={field.id}>
                ...
                <div className="ownerFieldBody">
                  ...
                  <div className="fieldActions ownerFieldActions">
                    {isOwnerPortal &&
                      (canManualBook ? (
                        <Link className="btn smallBtn" to={fieldLink(manualBookingPath, field)}>
                          Đặt tay trên sân này
                        </Link>
                      ) : (
                        <button type="button" className="btn smallBtn" disabled>
                          Chưa thể đặt sân này
                        </button>
                      ))}

                    {canOpenPublic && (
                      <a className="outlineBtnLink" href={publicBookingUrl} target="_blank" rel="noreferrer">
                  ...
                  {(manualHint || deletionState.reason) && (
                    <div className="ownerFieldNoteBox">
                      ...
                      <div className="ownerAdminMiniCard">
                        {manualHint && <span>{manualHint}</span>}
                        {deletionState.reason && <span>{deletionState.reason}</span>}
                      </div>
                    </div>
                  )}
                </div>
...
/* const ManagedBookingsSection = ({
  ...
  <section className="ownerSectionCard ownerSectionCard--bookings" id={ownerBookingsSectionId}>
    <div className="ownerSectionHeading">
      <div className="ownerSectionTitle">
        <p className="helperText">
          Đang bổ sung lịch khóa sân theo ngày {formatBookingDateLabel(managedBookingsDate)}.
        </p>
        <p className="ownerSectionEyebrow">Đơn đặt của khách</p>
        <h2>Quản lý đơn đặt của khách</h2>
      </div>
      <label className="adminDateFilter" htmlFor="owner-managed-bookings-date">
        <span>Ngày xem</span>
        <input
          id="owner-managed-bookings-date"
          type="date"
          value={managedBookingsDate}
          onChange={(event) => handleManagedBookingsDateChange(event.target.value)}
        />
      </label>
    ...
    {loading ? (
      <p>Đang tải đơn đặt...</p>
    ) : managedBookings.length === 0 ? (
      <>
      <p className="usersEmptyState">
        Chưa có đơn đặt nào trong danh sách quản lý của chủ sân cho ngày {formatBookingDateLabel(managedBookingsDate)}.
      </p>
            ...
            <article className="ownerBookingCard" key={booking.id}>
              <div className="ownerBookingHeader">
                <div>
                  ...
                  <p>
                    {booking.subFieldName || "Sân tổng"} | {formatBookingDateLabel(booking.date)} |{" "}
                    {booking.timeSlot}
                  </p>
                </div>
              ...
              <div className="fieldActions ownerFieldActions">
                {bookingStatus === "pending" && (
                  ...
                  >
                    {isProcessing && processingBookingAction === "confirm"
                      ? "Đang xác nhận..."
                      : "Xác nhận đặt sân"}
                  </button>
                )}
                ...
                {bookingStatus !== "cancelled" &&
                  ...
                  paymentStatus !== "deposit_paid" && (
                    ...
                    >
                      {isProcessing && processingBookingAction === "deposit"
                        ? "Đang xác nhận cọc..."
                        : "Xác nhận đặt cọc"}
                    </button>
                  )}
                ...
                {bookingStatus !== "cancelled" && paymentStatus !== "paid" && (
                  ...
                  >
                    {isProcessing && processingBookingAction === "payment"
                      ? "Đang xác nhận..."
                      : "Xác nhận thanh toán"}
                  </button>
                )}
                ...
                {bookingStatus !== "cancelled" && (
                  ...
                  >
                    {isProcessing && processingBookingAction === "cancel"
                      ? "Đang hủy..."
                      : "Hủy đơn đặt"}
                  </button>
                )}
    ...
    return (
      <section className="page section adminDashboardPage ownerDashboardPage">
        ...
        <div className="container ownerDashboardStats">
          {statsCards(stats).map(([label, value], index) => (
            <article className="adminStatCard ownerStatCard" key={`${label}-${index}`}>
              <span>{label}</span>
              <strong>{value}</strong>
            </article>
          ))}
        ...
        <div className="container">
          <QuickActionGrid manualBookingPath={manualBookingPath} />
        </div>
